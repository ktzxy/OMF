...
| `omf sql import <dump> [--remap 源[:目标]] [--check]` | 数据泵导入到应用模式 |
...
> 每个操作标注风险等级：🟢 低风险（只读/预览）｜🟡 需确认（有轻微动作，命令自带 confirm）｜🔴 高风险（停机/写库/改配置，需维护窗口）。
> 基于 B 组实测（详见 [docs/TEST_REPORT.md](docs/TEST_REPORT.md)）。

### 场景一：日常巡检（🟢 全部只读，可随时跑）
```bash
omf status                         # 库/监听/磁盘/备份/日志 一键总览
omf check all                     # 健康检查
omf listener status               # 监听运行 + 端口
omf backup list                   # 备份列表 + 过期高亮
omf clean logs -p                 # 各分类清理预览（不删除）
omf sql status                    # SQL 执行记录
omf sql scan                      # 待执行脚本
omf sql run "SELECT ..."          # 只读查询（自动切 PDB 执行）
```

### 场景二：变更 / 维护前备份（🟢→🟡）
```bash
omf backup full                   # 逻辑全量（变更前必做）
# 或 omf backup physical         # RMAN 物理全量
omf check all                    # 确认健康后再操作
```

### 场景三：重启监听器（🟡 业务连接中断几秒）
```bash
omf listener restart
# 收尾必做：确认服务已注册（restart 不会刷新 local_listener，需等库 OPEN 后 PMON 注册）
lsnrctl status                   # Services Summary 应含 ARTERYPDB 且 status READY
# 若 services 为空：sqlplus / as sysdba -> ALTER SYSTEM REGISTER;
```

### 场景四：重启数据库（🔴 业务停机，维护窗口执行）
```bash
omf db restart
omf db status                    # 确认 OPEN / ARCHIVELOG / PDB READ WRITE
omf listener status              # 确认监听服务已重新注册
```

### 场景五：日志与清理（🟡 会删旧文件，命令自带确认）
```bash
omf log clean                    # 清 N 天前日志（确认后执行）
omf clean logs -p                # 先预览，再决定是否 clean
omf sql rollback --all          # 仅删执行标记文件，不碰库内数据（让 init 可重跑）
```
...
omf db start / omf db stop                       # 起停库
```

### 场景七：高风险操作（🔴 非必要时不做）
- `omf listener port <新端口>`：改 `listener.ora`/`tnsnames.ora` + 防火墙 + 改 `local_listener` + 重启，配错可能连不上库。改前先备份 `listener.ora`/`tnsnames.ora`。
...
- **目录 OS 权限**：`oracle_dumps` 指向的 `ORACLE_DUMP_DIR`（`omf sql init` 已自动建好并归属 `oracle:oinstall`，权限 `750`）。若手动改动过，确认：`ls -ld $(omf config show 2>/dev/null | grep DUMP_DIR || echo /data/oracle/oracle_dumps)` 或 `ls -ld /data/oracle/oracle_dumps`，须为 `oracle:oinstall` 可读写，否则 impdp 报 `ORA-27037` / `permission denied`。
