...
| `omf clean logs [-d N \| --all]` | 清理日志（默认按 `LOG_RETENTION_DAYS`；`-d N` 删 N 天前；`--all` 删全部，需确认） |
| `omf clean trace [-d N \| --all]` | 清理 trace 文件（同上） |
| `omf clean audit [-d N \| --all]` | 清理审计文件（同上） |
| `omf clean archive [-d N \| --all]` | 清理归档日志（`-d N` 删 N 天前；`--all` 删全部，需确认） |
