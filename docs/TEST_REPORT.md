...
> ⚠️ **历史测试快照**：本文档记录 v1.5.0 时在本环境的实测结论，**可能与当前代码/文档脱节**（框架已迭代到 v1.58，部分坑点已修复、命令行为已变化，如 cron 用户、broker 自动建等）。排障请以 [TROUBLESHOOT.md](TROUBLESHOOT.md) 为准。
> 目的：记录各分组命令在早期版本的实测结论、风险分级与关键坑点，作为历史参考。
> 测试框架版本：OMF v1.5.0
