...
cmd_help() {
    case "${1:-}" in
        ...
        check)      echo "用法: omf check {all|db|disk|perf|alert|listener|preflight|schemas|dg|monitor}"; echo "  schemas  校验已配置模式(多库)是否真实存在于数据库"; echo "  dg       Data Guard 健康检查 (传输/MRP/延迟/间隙, 需 ENABLE_DG=true)"; echo "  monitor prom --push <pg_url> [--job x] [--instance x]  指标推 Pushgateway (对接 Prometheus)";;
