...
cmd_help() {
    case "${1:-}" in
        ...
        sql)        echo "用法: omf sql {scan|run|init|status|rollback}"; echo "  init [--schema <模式名>]            # 初始化; --schema 仅重建该模式(用户/表空间), 不重跑全局脚本"; echo "  import <dump> [--schema 模式名] [--remap 源[:目标]] [--remap-tablespace 源TS:目标TS] [--check] [--apply [parfile]]"; echo "    --schema 指定导入到的目标模式(多库多模式); 不指定则默认主模式 APP_USER"; echo "  rollback <name> | --all [--schema <模式名>]   # 重置执行记录; --schema 仅清该模式的记录";;
        tune)       echo "用法: omf tune {memory|storage|session|analyze|awr|apply}";;
        check)      echo "用法: omf check {all|db|disk|perf|alert|listener|preflight|schemas|monitor}"; echo "  schemas  校验已配置模式(多库)是否真实存在于数据库";;
